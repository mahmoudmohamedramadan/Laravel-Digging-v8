...
Route::get('relationships', function () {
    ...
    /* In the new version in Laravel, instead of saying `PhoneNumber::where('user_id', $user->id)->get()` you can use the next line */
    ...
    /* NOTE that: when you save, you must save an eloquent instance model, whereas in case you will use `create` or `createMany` you can pass an array */

    ##################################

    /* NOTE that: all the previous saving data was through the parent table to the child tablr, BUT what if we want to save data from child to parent...let's take a look at the next example */
    ...
    // We make the `user_id` equals NULL in the `phone_numbers` table using `phone_numbers.id` and `phone_numbers.user_id`
    // $user = User::find(2);
    ...
    // We will check if the user has a `phoneNumbers` where the `user_id` equlas to 5
    // $users = User::whereHas('phoneNumbers', function($q) {
    //     return $q->where('user_id', 5);
    // })->get();

    // There is an easy manner for the upper example
