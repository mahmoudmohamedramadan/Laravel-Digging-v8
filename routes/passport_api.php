...
/* via this route you can create token for first user to use it in login process, NOTE that you use `accessToken` you must create a personal token using `php artisan passport:install` this command creates a two keys in `storage` folder, also creates a two records into database one is `Laravel Personal Access Client` and the other is `Laravel Password Grant Client`, or you can run `php artisan passport:client --personal`  */
...
/* ## Password grant: The password grant, while less common than the authorization code grant, is much simpler. If you want users to be able to authenticate directly with your API using thier username and password

in order to use the password grant flow, you need a password grant client in your database.this is because every request to an OAUTH server needs to be made by a client*/
Route::get('/passport/password_grant', function () {
    /* when you deal with password grant do NOT forget to set `password_client` to 1, the `$url` in the parameter in the `post` method is the url inside this application, `client_id` is the value that exists in `oauth_access_tokens` table */
