...
/* if you have many URIs but different in HTTP verb only you can use `match` in that case */
...
Route::resource('user_new_features', NewFeaturesController::class)
    ->middleware('web')
    /* you can exclude some resource functions from your route using `except` */
    ->except(['edit', 'update', 'destroy'])
    /* `name` allows you to change the route's name of any route, EX: to access create route name you will not use `route('newControllers.create)` but you will use `route('user_new_features.user_new_feature)` */
    ...
    /* `scoped` in a `resource` method used to specify that segement name `user_feature` -we use `user_feature` becuase we have changed the parameter name using `parameters` method-, will be returned WITH an `email` instead of `id` */
    ->scoped([
        'user_feature' => 'email',
    ]);
