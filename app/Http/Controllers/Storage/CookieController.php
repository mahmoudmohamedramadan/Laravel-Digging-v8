...
{
    /* Cookies work the same as `sessions` and `caches`, Cookies can exist in three places in Laravel, they can come in via the request, which means the user had the cookie when they visited the page. You can read that with the cookie facade, or you can read it from the request object. They can also be sent out with a response, which means the response will instruct the user's browser to save the cookie for future visits. you can add the cookie to your response object before returning it. And lastly, a cookie can be queued. If you use the cookie facade to set a cookie, you have put it into a "CookieJar" queue, and it will be removed and added to the response object by the `AddQueuedCookiesToResponse` middleware */
    ...
    {
        // The `get` method to pull the value of a cookie that came in with the request
        Cookie::get('$key');

        // You can check whether cookie came in with the request using the `has` method
        Cookie::has('$key');
