...
{
    /* in a line at the bank. even if there are multiple lines-queues-only one person is being served at a time from each queue, and each person will eventually reach the front and be served. in some banks, it's strict FIFO sort of policy, but in other banks, there's not an exact guarantee that someone won't cut ahead of you in line at some point. Essentially, someone can get added to the queue, be removed from the queue prematurely, or be successfully `processed` and then removed.

    Queues in programming are very similar. your application adds a `job` to a queue, which is a chunk of code that tells the application how to perform a particular behavoir.

    Queues workers can delete the jobs, return them to the queue with a delay, or mark them as successfully processed.

    Laravel makes it easy to serve your queues using `Redis`, `beanstalkd`, `Amazon's Simple Queue Service (SQS)` */
    ...
    /* Queued jobs can depending on the environment, take many shapes like arrays of data or simple strings. in laravel, hey will each be a collection of information containing the job name, the data payload, the number of attempts that have been made so far to process this job, and some other simple metadata.

    But you do NOT need to worry about any of that in your interactions WITH laravel, Laravel provides a structure called a Job, which intended to encapsulate a single task- a behavior that your application can be commanded to do and allow it to be added and pulled from a queue. */
    ...
    public function index()
    {
        ...
        /* also you can customize the amount of time your queue workers should wait before processing a job WITH the `delay()` method, which accepts either an integer representing the number of seconds to delay a job or a DateTime/Carbon instance */
        ...
        **Exceptions in handling** if an exception is thrown, the queue listener will release that job back onto the queue. The job will be rereleased to be processed again and again until it is able to finish successfully or until it has been attempted the maximum number of times allowed by your queue listener */
        ...
        /* any job that has surpassed its maximum number of allowed attempts will be dumped there, BUT there are quite a few things you can do WITH your failed jobs

        First, you can define a `failed` method on the job itself, which will run when that job fails */
    ...
    public function davingQueues()
    {
        ...
        Mail::to('mahmoud@laravel.com')->send(new FirstMail(request()->user));
